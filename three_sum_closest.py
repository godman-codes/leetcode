class Solution:
    def threeSumClosest(self, nums, target):
        ...
        count = 0
        if target < abs(nums[-1]*3) or target < abs(nums[1]*3):
            for i in range(len(nums)-2):
                ...
                while l < r:
                    count+=1
                    cumulative = nums[i] + nums[l]+ nums[r]
                    previous_difference = abs(res-target)
                    new_diff = abs(cumulative-target)
                    print(previous_difference)
                    print(new_diff)
                    if (new_diff) == 0:
                        print(count)
                        return cumulative
                    if new_diff < previous_difference:
